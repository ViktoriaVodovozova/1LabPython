...
def run_server(port: int = 8191) -> None:
    """Запускает HTTP сервер."""
    with socketserver.TCPServer(("", port), MyRequestHandler) as httpd:
        logger.info(f"Сервер запущен на порту {port}")
        logger.info("Доступные маршруты:")
        logger.info("  http://localhost:8191/ - Главная страница")
        logger.info("  http://localhost:8191/users - Список пользователей")
        logger.info("  http://localhost:8191/user?id=1 - Информация о пользователе")
        logger.info("  http://localhost:8191/currencies - Курсы валют")
        logger.info("  http://localhost:8191/author - Информация об авторе")
        logger.info("  http://localhost:8191/currency/delete?id=1 - Удаление валюты")
        logger.info("  http://localhost:8191/currency/update?USD=78.5 - Обновление курса")
